## python version: 3.7
## pip install: requests, aiohttp, aiohttp_socks, lxml
...
class Netstream():
    ...
    def finish(self):
        ...
        logging.debug(f'finished at {time_total}')
    
    async def get_page(self, client, target):
        async with client.get(target) as response:
            if response.status != 200:
                logging.debug(f'no response from url: {target}')
                return ''
        ...
        while (attemp <= self.attemps):
            ...
            try:
                ...
                async with aiohttp.ClientSession(loop=loop, headers=headers, connector=connector, timeout=timeout) as client:
                    logging.debug(f'receiving page content: {target} ({attemp}/{self.attemps} {proxy})')
                    self.data_content[target] = ''
                    data = await self.get_page(client, target)
                    self.data_content[target] = data.decode('utf-8')
                    logging.debug(f'!received page content: {target} ({attemp}/{self.attemps} {proxy})')            
                    self.data_received += 1
                    logging.debug(f'total progress: {self.data_received}/{self.data_total}')
                    return
            except:
##                logging.debug(traceback.format_exc())
    ...
    def get_proxies(self):
        logging.debug(f'receiving proxies...')
        try:
...
##            logging.debug(f'proxies: {self.proxies}')
            logging.debug(f'!received proxies: {len(self.proxies)}')
        except:
##            logging.debug(traceback.format_exc())
            pass
